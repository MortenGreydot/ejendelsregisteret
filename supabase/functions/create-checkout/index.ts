    ...
    try {
      ...
      try {
        ...
        return Response.json(
          {
            error:
              "Forespørgslen kunne ikke læses. Genindlæs siden og prøv igen.",
          },
          { status: 400 },
        );
      }
      ...
      if (readError) {
        console.error(
          "create-checkout: kunne ikke læse abonnement:",
          readError,
        );
        return Response.json(
      ...
      // Én bruger, ét abonnement.
      //
      // Statussen i vores egen tabel er ikke nok til at afgøre det. Den
      // sættes af webhooken og kan være et hak bagud, og "past_due" er ikke
      // active — men abonnementet lever i høj grad stadig hos Stripe. Uden
      // det her tjek kunne en kunde med en fejlet betaling trykke
      // "Prøv betalingen igen" og få et NYT abonnement ved siden af det
      // gamle: to abonnementer, to træk hver måned.
      //
      // Derfor spørges Stripe direkte, når vi kender et abonnements-id.
      // Kun de statusser hvor abonnementet er endeligt slut må slippe
      // videre til en ny checkout.
      const AFSLUTTEDE = ["canceled", "incomplete_expired"];

      if (existing?.stripe_subscription_id) {
        let live = true;
        try {
          const nuvaerende = await getStripe().subscriptions.retrieve(
            existing.stripe_subscription_id,
          );
          live = !AFSLUTTEDE.includes(nuvaerende.status);
        } catch (caught) {
          // Kender Stripe ikke id'et, er der intet at dublere. Enhver
          // anden fejl behandles som "lever" — vi må hellere afvise en
          // gyldig ny tegning end oprette et abonnement nummer to.
          const kode = (caught as { code?: string })?.code;
          live = kode !== "resource_missing";
          if (live) {
            console.error("create-checkout: kunne ikke tjekke abonnement:", caught);
          }
        }
        ...
        if (live) {
          return Response.json(
            {
              error:
                "Du har allerede et medlemskab. Skal betalingsoplysningerne opdateres, kan du gøre det under Min side.",
            },
            { status: 409 },
          );
        }
      ...
      if (upsertError) {
        console.error(
          "create-checkout: kunne ikke gemme abonnement:",
          upsertError,
        );
        return Response.json(
